...
contract WIDEANGLE is ERC721, ReentrancyGuard {
    ...
    ERC721(name, symbol)
    {
        ...
        WIDEANGLE_ADMIN_WALLET=0x985376c953ab63F7C5466D82a2EE2341dF76a90d;
        _tokenIdCounter+=1;
        _base_URI = baseURI;
        sellIsClose=1;
        mintFunctionsState[0]=1;
        mintFunctionsState[1]=0;
        mintFunctionsState[2]=0;
        stableorEth= 0;
    }
...

    {
        ...
        for(uint256 i = 0 ;i < categories.length ; i++){
            price += categoryId[categories[i]].privateSalePrice * 10**18;
        }
    ...
    payable
    {
        ...
        if(stableorEth==0){
            for(uint256 i = 0 ;i < _categories.length ; i++){
                price += categoryId[_categories[i]].preSalePrice * 10**18;
            }
            USDT.transferFrom(_msgSender(),WIDEANGLE_ADMIN_WALLET,price);
        }
        else{
    ...
    payable
    {
        ...
        if(stableorEth==0){
            for(uint256 i = 0 ;i < categories.length ; i++){
                price += categoryId[categories[i]].publicSalePrice * 10**18;
            }
            USDT.transferFrom(_msgSender(),WIDEANGLE_ADMIN_WALLET,price);
        }
        else{
